...
class GptBytePairEncoding implements Encoding {
    ...
    final CompactTokenEncoder compactEncoder;
    ...
    int encodeOrdinaryInternal(String text, int maxTokenCount, boolean keepEncodings, IntArrayList out) {
        ...
        for (Matcher matcher = pattern.matcher(text); tokenCount < maxTokenCount && matcher.find(); ) {
            ...
            tokenCount += processTokens(maxTokenCount, keepEncodings, out, ranks, bytes, bytes.length);
        }
        return tokenCount;
    }

    int processTokens(int maxTokenCount, boolean keepEncodings, IntArrayList out, IntArrayList ranks, byte[] bytes, int size) {
        if (CompactTokenEncoder.accepts(size)) {
            return compactEncoder.addTokensAndGetCount(maxTokenCount, keepEncodings, out, ranks, bytes, size);
        } else {
            assert TokenEncoder.accepts(size);
            return encoder.addTokensAndGetCount(compactEncoder, maxTokenCount, keepEncodings, bytes, size, out, ranks);
        }
    }
    ...
    public String decode(IntArrayList tokens) {
        var decoded = decodeBytes(tokens);
