...
class GptBytePairEncodingTest {
    ...
    void bytePairMerge3() {
        ...
        var sum1 = 0;
        for (var x : Cl100kTest.getTexts("../")) {
            sum1 += encoding.encode(x).size();
        }
        System.out.println(sum1);
        assertEquals(25_876_703, sum1);

        var ranks = loadMergeableRanks("cl100k_base.tiktoken");

        var sizes = 0;
        var skipped = new ArrayList<>();
        var collect = ranks.entrySet().stream().sorted(comparingInt(a -> a.getKey().length)).toList();
...
//                System.out.println("Testing: " + stringToken);
                ...
                assertEquals(intToken, encodedTokens.get(0), "Encoded tokens do not match expected value for stringToken: " + stringToken);
