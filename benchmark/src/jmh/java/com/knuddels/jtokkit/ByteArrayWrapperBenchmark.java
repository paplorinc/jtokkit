...
public class ByteArrayWrapperBenchmark {
    private List<ByteArrayWrapper> byteArrays;
    ...
    public void setup() {
        ...
        for (var size = 8; size <= 100; size++) {
            ...
            for (var i = 8; i <= size; i++) {
                byteArrays.add(new ByteArrayWrapper(bytes, 0, i));
            }
            for (var i = 8; i + Long.BYTES < size; i++) {
                byteArrays.add(new ByteArrayWrapper(bytes, i, bytes.length));
            }
        }
    }
